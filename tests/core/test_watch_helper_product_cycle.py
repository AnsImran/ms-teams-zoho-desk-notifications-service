...
import os                                # Build expected state-file paths.
...
class _FakeFuture:
    ...
    def __init__(
        ...
        *,
        ...
        self._value = value  # Value returned by .result().
        self._error = error  # Optional error raised by .result().
    ...
    *,
    ...
    class _FakeExecutor:
        def __init__(self, max_workers=None):
            self.max_workers = max_workers  # Preserve submitted max_workers for assertions.
...
def test_run_single_product_cycle_routes_to_normal_webhook_when_magic_not_matched(monkeypatch, product_config, fixed_now) -> None:
    ...
    assert posted == [
        {
            "webhook_url": "https://teams.example/webhook",
...
def test_run_single_product_cycle_raises_when_send_future_fails(monkeypatch, product_config, fixed_now) -> None:
    ...
    _install_fake_executor(
        monkeypatch,
        submit_error = RuntimeError("send failed"),
    )
...
def test_run_product_loop_once_builds_path_delegates_and_saves(monkeypatch, product_config) -> None:
    """run_product_loop_once should load, delegate, and save when changed=True."""  # Branch: changed save path.
    expected_path                         = os.path.join(os.path.dirname(os.path.abspath(watch_helper.__file__)), product_config.last_sent_filename)
    loaded_last_sent: Dict[str, datetime] = {"1166045000005103001": datetime(2026, 1, 1, 12, 0, 0)}
    ...
    watch_helper.run_product_loop_once(
        product_config,
        token               = "token-123",
        pre_fetched_tickets = pre_fetched,
    )
